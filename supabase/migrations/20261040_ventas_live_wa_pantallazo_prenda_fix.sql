-- 20261040_ventas_live_wa_pantallazo_prenda_fix.sql
-- Ventas Live · WhatsApp · FIX del pantallazo de prenda (reporte real del dueño)
--
-- QUÉ PASÓ (evidencia en la conversación de producción, 2026-09-12):
--   00:33:52  in  "Holis"            → out "holis bonit@"            ✓
--   00:34:34  el negocio carga la prenda (vl_items "Polera" $5.000)  ✓
--   00:34:54  in/imagen (pantallazo) → SIN respuesta + aviso "comprobante"  ✗
--   00:35:15  in "Quiero pagar"      → out "¿quieres datos de pago o espero el comprobante bonit@?"
--
-- CAUSA: la condición de "ya se le mandaron los datos de pago" usaba
-- `vl_wa_chats.datos_pago_enviado_en`, que en un cliente HABITUAL quedó marcado
-- desde la conversación ANTERIOR (días antes). Con eso, TODOS sus pantallazos se
-- interpretaban como comprobante de pago → el bot no confirmaba la prenda.
--
-- ARREGLO: el pantallazo se decide con datos del chat, sin mirar
-- `datos_pago_enviado_en`:
--   * es PRENDA  → el negocio acaba de cargar la prenda (la última prenda del
--     pedido es posterior al último mensaje del chat), o no hay ninguna
--     conversación de pago en curso (el bot no está esperando el comprobante).
--   * es COMPROBANTE → el bot lo está esperando (el último mensaje del bot habla
--     de "comprobante" o hay un aviso abierto `esperando_comprobante`), o el
--     mensaje mismo dice que es un comprobante.
-- El aviso `prenda` ahora distingue si la prenda YA está cargada o si hay que
-- cargarla (el negocio pidió que el pantallazo sirva de respaldo de la prenda).
--
-- Sin cambios de esquema: solo el cerebro (el guion vive en la BD → live con
-- `supabase db push --linked`, sin deploy de Vercel).
...
SELECT '[VENTAS LIVE] Fix pantallazo de prenda (ya no lo confunde con comprobante en clientes habituales) OK' AS status;
