...
// CRUD contra tabla subscriptions en Supabase (columnas: status, start_date, end_date)
...
export async function crearSuscripcion(plan, tenantId, periodoMeses = null) {
    ...
    if (error) throw new Error('Error al crear suscripcion: ' + error.message);

    await getSupabase()
        .from('tenants')
        .update({ plan })
        .eq('id', String(tenantId).trim());
...
export async function getAllSuscripciones() {
    try {
        ...
        return (data || []).map(s => ({
            ...
            negocio: s.tenants?.nombre_negocio || 'N/A',
            email: s.tenants?.email_contacto || '',
            plan: s.plan,
            estado: s.status || 'inactive',
...
export async function actualizarPlan(tenantId, nuevoPlan) {
    ...
    await getSupabase().from('tenants').update({ plan: nuevoPlan }).eq('id', String(tenantId).trim());
...
export async function tieneAccesoPremium() {
    ...
    return sub.plan === 'pro' || sub.plan === 'premium_anual';
}