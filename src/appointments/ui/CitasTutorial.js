// ============================================================
// TUTORIAL EN VIDEO — sección Citas Programadas (admin)
...
// Mismo patrón que initTutorialServicio (ServiceForm.js) e
...
// Adaptación estructural: el h3 de citas NO es hijo directo del
// .glass-panel — vive dentro de una fila flex con la leyenda de
// iconos. El reproductor se inserta DESPUÉS de esa fila y el
// botón se agrupa con el h3 en un div flex simple (la leyenda
// queda intacta a la derecha).
// ============================================================
const VIDEO_TUTORIAL_CITAS_URL = 'https://dfcfimipkfhitlsyixqu.supabase.co/storage/v1/object/public/tutoriales/tutorial-citas-programadas.mp4?v=1';

export function initTutorialCitas() {
    const section = document.getElementById('section-citas');
    if (!section) return;
    const header = section.querySelector('.glass-panel h3');
    if (!header) return;
    // Fila flex que ya contiene h3 + leyenda de iconos (admin.html)
    const headerRow = header.parentNode;
    if (!headerRow) return;

    // ---- Contenedor del reproductor (después de la fila del header) ----
    const wrap = document.createElement('div');
    wrap.className = 'tutorial-video-wrap';
    wrap.id = 'tutorial-video-wrap-citas';
    wrap.innerHTML = `
        <div class="tutorial-drag-bar" id="tutorial-drag-bar-citas" title="Arrastrá para mover"><i class="fas fa-grip-vertical"></i> <span>Arrastrá el video para moverlo</span></div>
        <div class="tutorial-bar">
            <video id="tutorial-video-citas" controls playsinline preload="metadata"></video>
        </div>
        <p class="tutorial-fixed-hint"><i class="fas fa-arrow-down"></i> Usá los botones de acá abajo — el tutorial sigue arriba</p>
        <button type="button" class="tutorial-close-btn" id="tutorial-close-btn-citas" title="Cerrar tutorial" aria-label="Cerrar tutorial"><i class="fas fa-times"></i></button>
        <button type="button" class="tutorial-zoom-btn" id="tutorial-zoom-btn-citas" title="Agrandar video" aria-label="Agrandar video"><i class="fas fa-expand"></i></button>
        <div class="tutorial-msg" id="tutorial-msg-citas"></div>
    `;
    headerRow.parentNode.insertBefore(wrap, headerRow.nextSibling);
    ...
    // ---- Botón "Ver tutorial" junto al título (grupo flex simple) ----
    const grupo = document.createElement('div');
    grupo.style.cssText = 'display:flex;align-items:center;gap:10px;min-width:0;';
    headerRow.insertBefore(grupo, header);
    grupo.appendChild(header);

    const btn = document.createElement('button');
    btn.type = 'button';
    btn.id = 'btn-ver-tutorial-citas';
    ...
    function abrirTutorial() {
        if (!video.src) video.src = VIDEO_TUTORIAL_CITAS_URL;
    ...
    function cerrarTutorial() {
        ...
        // Restaurar el wrap a su lugar original (después de la fila del header)
        if (wrapParent) {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (item && item.dataset.section && item.dataset.section !== 'citas') {
            cerrarTutorial();
        }
    });
