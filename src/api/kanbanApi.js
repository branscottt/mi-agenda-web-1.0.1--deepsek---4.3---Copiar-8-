...
//         kanban_checklist_items, kanban_attachments.
...
/**
 * Carga lists + cards + checklist + adjuntos del board en paralelo
 * y los devuelve anidados:
 * { lists: [{ id, titulo, posicion, cards: [{ id, titulo, descripcion,
 *   posicion, etiquetas, cita_id, checklist: [], adjuntos: [] }] }] }
 */
export async function getBoardData(boardId) {
    ...
    const [checklistRes, adjuntosRes] = await Promise.all([
        cardIds.length
    ...
    const checklistPorCard = {};
    checklist.forEach(it => {
        if (!checklistPorCard[it.card_id]) checklistPorCard[it.card_id] = [];
        checklistPorCard[it.card_id].push(it);
    });
    ...
    return {
        lists: lists.map(l => ({
            ...
            cards: cardsList
                ...
                .map(c => ({
                    ...c,
                    checklist: checklistPorCard[c.id] || [],
...
export async function addChecklistItem(cardId, texto, posicion) {
    const supabase = getSupabase();
    const { data, error } = await supabase
        .from('kanban_checklist_items')
        .insert({ card_id: cardId, texto, posicion })
        .select()
        .single();
    if (error) throw error;
    return data;
...
/** Sube el binario al bucket privado y devuelve el storage_path. */
export async function uploadAttachment(file, boardId, cardId) {
    const supabase = getSupabase();
