...
export async function getAllServicios(tenantId) {
    ...
    return cacheWrapper(CACHE_PREFIX, async (tid) => {
        const { data, error } = await getSupabase()
            .from(TABLE)
            .select('id, tenant_id, nombre, descripcion, precio, duracion, imagen, activo, destacado, categoria, disponibilidad, fechas, assignment_mode, weekday_modules, date_specific_modules, module_date_cupos, tipo_venta, precio_individual, num_sesiones, precio_promocion, created_at')
