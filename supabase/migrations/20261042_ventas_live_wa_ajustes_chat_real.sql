-- 20261042_ventas_live_wa_ajustes_chat_real.sql
-- Ajustes salidos de correr el CHAT REAL (el de "Anubis") contra el bot, paso a paso.
-- Tres correcciones concretas, medidas en el probe:
--
--   F1. El courier suelto ("por blue por favor") ya no dispara el aviso "no supe qué
--       responder": el bot SÍ entendió (guardó el courier) y solo está esperando los
--       datos. Antes avisaba en falso.
--   F2. El paso de datos de envío solo se cierra con DATOS DE VERDAD: correo o
--       teléfono, o un texto con número y varias palabras (una dirección), o con el
--       courier y varias palabras. Antes bastaba "25 caracteres", así que la charla
--       ("Mis datos", "Déjeme hacerle su pago") cerraba el paso SIN datos y después
--       la dirección de verdad se perdía (el chat ya no la miraba).
--   F3. "no e podido transferir / no puedo pagar / me bloqueó la app" YA NO se avisa
--       como "el cliente dice que pagó" (era al revés). Ahora cae en el aviso general
--       "No se entendió" para que lo responda una persona (lo que pidió el dueño).
--
-- (El resto sigue igual que en 20261041.)
